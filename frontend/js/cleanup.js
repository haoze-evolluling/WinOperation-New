...
async function doScanCleanup() {
    ...
    if (result.status !== "ok") { cleanupResultEl.innerHTML = `<div class="result-card glass error"><strong>错误</strong> — ${result.error || '操作失败'}</div>`; return; }
...
function renderCleanupResult(data) {
    ...
    cleanupResultEl.innerHTML = html;
}
